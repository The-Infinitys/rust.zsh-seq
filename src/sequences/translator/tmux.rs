...
pub fn to_tmux_string(sequence: &TermSequence) -> String {
    match sequence {
        ...
        TermSequence::CurrentDirectoryFull => "#{pane_current_path}".to_string(),
        TermSequence::CurrentDirectoryTilde => "#{pane_current_path}".to_string(), // Tmuxはパス置換が複雑なため
